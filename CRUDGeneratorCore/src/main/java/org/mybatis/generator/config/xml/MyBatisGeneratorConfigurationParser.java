...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseJavaServiceGenerator(Context context, Node node){
        JavaServiceGeneratorConfiguration javaServiceConfiguration=new JavaServiceGeneratorConfiguration();
        context.setJavaServiceGeneratorConfiguration(javaServiceConfiguration);
        Properties attributes = parseAttributes(node);
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$

        javaServiceConfiguration.setTargetPackage(targetPackage);
        javaServiceConfiguration.setTargetProject(targetProject);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseProperty(javaServiceConfiguration, childNode);
            }
        }
    ...
    private void parseJavaControllerGenerator(Context context, Node node){
        JavaControllerGeneratorConfiguration javaControllerConfiguration=new JavaControllerGeneratorConfiguration();
        context.setJavaControllerGeneratorConfiguration(javaControllerConfiguration);
        Properties attributes = parseAttributes(node);
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$

        javaControllerConfiguration.setTargetPackage(targetPackage);
        javaControllerConfiguration.setTargetProject(targetProject);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseProperty(javaControllerConfiguration, childNode);
            }
        }
